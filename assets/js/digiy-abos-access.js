/* ============================================================
   DIGIYLYFE · JOBS · ABOS ACCESS BRIDGE
   À poser dans : assets/js/digiy-abos-access.js

   Rôle :
   - Préparer JOBS au rail commun ABOS / PAY / ADMIN.
   - Lire l’accès module via :
     public.digiy_has_module_access_from_abos(phone, module)

   Important :
   - JOBS est encore à cadrer dans le rail.
   - Ne pas bloquer JOBS avec protect() tant que le plan n’est pas actif.
   ============================================================ */
...
(function () {
  ...
  function savePhone(phone, module) {
    ...
    try {
      ...
      if (m === "JOBS") {
        localStorage.setItem("DIGIY_JOBS_PHONE", p);
        localStorage.setItem("DIGIY_JOBS_LAST_PHONE", p);
      }
    } catch (_) {}
  }
  ...
  function renderAccessBadge(target, result) {
    ...
    if (result.has_access) {
      el.innerHTML = `
        ...
        ${result.fiche_title || result.module || "JOBS DIGIY"}<br>
        <small>Expire : ${result.expires_at || "date suivie par DIGIY"}</small>
      `;
    } else {
      el.innerHTML = `
        <strong>🔒 JOBS à cadrer</strong><br>
        <small>Le pont ABOS est prêt. PAY garde la preuve, ADMIN validera quand le plan JOBS sera actif.</small>
      `;
    }
  }
